...
export default function BookCard({ book }) {
  const getConditionBadge = (condition) => {
    switch (condition) {
      case 'nou':
        return <span className="bg-[#10B981]/10 text-[#10B981] px-2 py-1 rounded text-xs font-medium border border-[#10B981]/20">Nou</span>
      case 'com_a_nou':
        return <span className="bg-[#3B82F6]/10 text-[#3B82F6] px-2 py-1 rounded text-xs font-medium border border-[#3B82F6]/20">Com a nou</span>
      case 'bon_estat':
        return <span className="bg-[#F59E0B]/10 text-[#F59E0B] px-2 py-1 rounded text-xs font-medium border border-[#F59E0B]/20">Bon estat</span>
      case 'acceptable':
  ...
  // Format language
  const formattedLanguage = book.idioma || 'Desconegut'
